...
{
    /// <summary>
    /// Convenience class for holding an instantiation of the CalcmasterFractalDLL's FractalGenerator class
    /// so that we don't need to use a pointer to call the FractalInterface static functions.
    /// It also cleans up the unmanaged instance of the FractalGenerator by calling
    /// FractalInterface.DestroyFractalGenerator when this class is garbage collected so that you don't have
    ...
    internal class Fractal : IDisposable
    {
        ...
        // Returns the last error code generated by the FractalGenerator class, 0 if no error has occurred.
        public int GetLastErrorCode()
        {
            return m_ptrFractalGenerator == IntPtr.Zero ? -1 : FractalInterface.GetLastErrorCode(m_ptrFractalGenerator);
        }

        // Returns an error message based on last generated error code
        ...
        public void SetDimensions(Rectangle bounds)
        {
            ...
            MessageBox.Show($"Fractal image size was reset to height={m_height} and width={m_width}");
        ...
        // number of random colors to generate in _randomColors list
        ...
        public Bitmap BitmapFromIterations() // with smoothing
        {
